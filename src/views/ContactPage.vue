      ...
      <div class="container">
        <div class="channels-grid">
          ...
          >
            ...
            <span class="channel-link">前往联系 →</span>
          </a>
        </div>
      </div>
    </section>
        ...
        <form class="contact-form" @submit.prevent="handleSubmit">
          <div class="form-row">
            <div class="form-field">
              ...
              <input id="cf-name" v-model="form.name" type="text" placeholder="怎么称呼你" required class="field-input" />
            </div>
            <div class="form-field">
          ...
          <div class="form-field">
            ...
            <input id="cf-subject" v-model="form.subject" type="text" placeholder="简单描述一下需求或问题" class="field-input" />
          </div>
...
const faqs = [
  {
    q: '一般多久回复？',
    a: 'QQ 通常更快，邮箱一般会在 24 小时内处理。若事情比较紧急，建议优先通过 QQ 联系。'
  },
  {
    q: '可以做定制开发吗？',
...
.page-hero { text-align: center; padding: calc(64px + 2rem) 0 2rem; }
.page-title { font-size: clamp(1.75rem, 3vw, 2.25rem); font-weight: 700; margin-bottom: 0.75rem; }
.title-underline { width: 48px; height: 3px; background: var(--gradient-primary); border-radius: 2px; margin: 0 auto 1rem; transform: scaleX(0); animation: ulSlideIn 0.5s ease 0.15s forwards; }
@keyframes ulSlideIn { to { transform: scaleX(1); } }
.page-subtitle { max-width: 620px; margin: 0 auto; color: var(--text-secondary); line-height: 1.7; }
...
.channel-name { margin: 0 0 0.35rem; color: var(--text-primary); font-size: 1.05rem; }
.channel-label { margin: 0 0 0.45rem; color: var(--accent); font-size: 0.88rem; }
.channel-desc { margin: 0; color: var(--text-secondary); line-height: 1.65; flex: 1; }
.channel-link { margin-top: 1rem; color: var(--accent); font-weight: 600; }

.form-container { max-width: 760px; }
.form-header { text-align: center; margin-bottom: 1.75rem; }
.form-title { margin: 0 0 0.45rem; font-size: 1.5rem; }
...
.field-label { display: block; margin-bottom: 0.5rem; color: var(--text-primary); font-size: 0.9rem; }
...
.sending { display: inline-flex; align-items: center; gap: 0.5rem; }
.spinner {
  width: 14px;
  height: 14px;
...
.section-title { margin-bottom: 1rem; text-align: center; font-size: 1.5rem; }
...
.faq-icon { flex: 0 0 auto; font-size: 1.15rem; }
